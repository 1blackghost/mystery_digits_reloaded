    ...
    try:
        ...
        if len(digits)!=len(guessed_digits):
            ...
            context = {
                        ...
                        'filepath': "/static/images/" + request.session["filename"],
            ...
            # Iterate through the guessed digits
            for guess in guessed_digits[:]:  # Use a copy to avoid modifying the list during iteration
                if guess in digits:
                    digits.remove(guess)
                    guessed_digits.remove(guess)  # Remove the digit from the digits list
            ...
            if len(final) == int(user_profile.lvl):
                ...
                digits_in_image = load_data_from_json(os.getcwd()+"\main\data.json")
...
def dashboard(request):
    ...
    if phone:
        ...
        if not request.session.get("generated"):
            ...
            'filename': "/static/images/" + str(filename),
            ...
            'filename': "/static/images/" + request.session["filename"],
